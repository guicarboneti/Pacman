...
int main()
{
    ...
    int fase, tecla = 0, vidas = 3, colide, aux1=1, aux2=1;
    ...
    for (fase = 1; (fase < 4); fase++)
    {
        if (fase == 1)
        {
            ...
            if (vidas == 3)
                vidas--;
            while (tecla!= 'q' && tecla != 'Q' && !encerrou(tabuleiro) && !(colisao(tabuleiro, &pacman) && pacman.energizado == 0) && vidas>=0 && colide == 0)
            {
                tecla = wgetch(stdscr);
                if (tecla == 'a' || tecla == 'd' || tecla == 'w' || tecla == 's' || tecla == KEY_LEFT || tecla == KEY_RIGHT || tecla == KEY_UP || tecla == KEY_DOWN)
                    mvprintw(38, 5, "%d VIDAS", vidas);
                movimentacao(tabuleiro, &pacman, fase, &tecla, ListaFantasmas, moedas, &sistema, 0, &colide);
            }
            if (tecla == 'q' || tecla == 'Q')
            {
                endwin();
            ...
            if (colisao(tabuleiro, &pacman) || colide == 1)
            {
                fase--;
                vidas--;
            }
