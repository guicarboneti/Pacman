...
void movimentacao(unsigned char tabuleiro[LINHAS][COLUNAS], struct pac *ptr_pac, int fase, int *ptr_tecla, struct lista_dupla_ligada *L, struct moeda moedas[152], struct sistema *ptr_sistema, int cont, int *colide)
{
    ...
    switch (*ptr_tecla)
    {
        ...
        case KEY_LEFT:      /* seta pra esquerda */
            ptr_pac->direcao = 1;
            while (!parede(tabuleiro, ptr_pac->x, ptr_pac->y, ptr_pac->direcao) && !encerrou(tabuleiro) && !(colisao(tabuleiro, ptr_pac) && ptr_pac->energizado == 0) && *colide == 0) /* verifica se a próxima posição é a parede */
            {
                ...
                else
                {
                    ...
                    {
                        reinicia_fantasma(tabuleiro, ptr_pac, L, colide);
                        ...
                        *ptr_tecla = wgetch(stdscr);
                        if (*ptr_tecla == 'd' || *ptr_tecla == KEY_RIGHT || *ptr_tecla == 'w' || *ptr_tecla == KEY_UP || *ptr_tecla == 's' || *ptr_tecla == KEY_DOWN)
                        {
                            ...
                            if (!parede(tabuleiro, ptr_pac->x, ptr_pac->y, direcao))
                                movimentacao(tabuleiro, ptr_pac, fase, ptr_tecla, L, moedas, ptr_sistema, cont, colide);
                            else
                                espera = 1; 
                        }
                    }
